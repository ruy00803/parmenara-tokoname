...
現在は4言語の店舗紹介・メニュー表示とメニューのmicroCMS連携を実装しています。店舗限定イベントの画面表示とCMS連携は、今後の拡張項目です。
...
| メニューのCMS連携 | microCMSから取得。未設定・取得失敗時は静的データに切替 |
| 店舗情報・イベント・お知らせのCMS連携 | 型定義と取得関数あり。画面には未接続 |

トップページの人気メニューは静的データを参照しています。CMSを更新しても、トップページの内容は自動では変わりません。
...
ブラウザで [日本語トップ](http://localhost:3000/ja) を開きます。microCMSを使わない場合、環境変数の設定は不要です。

CMSを利用する場合は `.env.example` を `.env.local` にコピーし、サービスドメインとAPIキーを設定してください。詳細は[更新・運用ガイド](docs/maintenance.md)を参照してください。
...
│   ├── i18n/               # 言語設定・ナビゲーション
│   ├── lib/microcms.ts      # CMS取得・静的データへの切替
│   └── types/microcms.ts    # CMSレスポンスの型定義
...
- 常滑店限定イベント・お知らせの表示とCMS連携
- 店舗情報とトップページの人気メニューのCMS連携
